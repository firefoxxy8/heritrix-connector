...
public class HttpAuthenticationCredential extends Credential {
    ...
    public static BasicSecurityManager securityManager = null;
    ...
    public String getPassword() {
      if (securityManager!=null){
        return securityManager.decryptString(this.password);
      } else {
    ...
    /** NTLM Auth realm. */
    String ntlmRealm = "ntlm";
    public String getNtlmRealm() {
        return this.ntlmRealm;
    }
    public void setNtlmRealm(String ntlmRealm) {
        this.ntlmRealm = ntlmRealm;
    }
    
    Boolean usingNtlm = null;
    public Boolean getUsingNtlm(){
      return this.usingNtlm;
    }
    public void setUsingNtlm(Boolean usingNtlm){
      this.usingNtlm = usingNtlm;
    }
    ...
    public boolean populate(CrawlURI curi, HttpClient http, HttpMethod method) {
        ...
        if (usingNtlm == null || !usingNtlm){
          ...
          result);
           ...
           NTCredentials ntc = new NTCredentials(getLogin(),getPassword(),getNtlmRealm(),getNtlmDomain());
            http.getState().setCredentials(new AuthScope(AuthScope.ANY), ntc);
    ...
    public static HttpAuthenticationCredential getByRealm(Set<Credential> rfc2617Credentials,
      ...
      if (rfc2617Credentials != null && rfc2617Credentials.size() > 0) {
        for (Iterator<Credential> i = rfc2617Credentials.iterator(); i.hasNext();) {
          ...
          if (realm==null || c.getRealm().equals(realm)) {
            result = c;
            break;
          }
        }
